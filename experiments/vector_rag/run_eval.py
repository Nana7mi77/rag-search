"""
向量检索 vs BM25 对比实验
使用 bge-m3 (HuggingFace Inference API) 做向量化，零本地依赖
"""
import csv
import json
import math
...
def best_snippet(text: str, query: str, max_len: int = 180) -> str:
    text = normalize(text)
    wanted = [t for t in cjk_ngrams(normalize(query)) if len(t) >= 2]
    if not text:
        return ""
    ...
    def score(s: str) -> int:
        return sum(1 for t in wanted if t and t in s)

    selected = max(sentences, key=score)
    if score(selected) == 0:
        selected = text
    return selected[:max_len].rstrip() + "..." if len(selected) > max_len else selected
...
def sf_embed(texts: List[str], retries: int = 3) -> List[List[float]]:
    if SF_API_KEY:
        return _embed_siliconflow(texts, retries)
    if HF_API_KEY:
        return _embed_hf(texts, retries)
    raise RuntimeError("需要设置 SF_API_KEY（硅基流动）或 HF_API_KEY（HuggingFace）环境变量")


def _embed_siliconflow(texts: List[str], retries: int = 3) -> List[List[float]]:
    headers = {
    ...
    req = request.Request(SF_API_URL, data=body, headers=headers, method="POST")
    opener = _make_opener()
    for attempt in range(retries):
        try:
            ...
            if e.code == 429 and attempt < retries - 1:
                ...
                time.sleep(wait)
                continue
            raise RuntimeError(f"SiliconFlow API error {e.code}: {detail}") from e
    raise RuntimeError("SiliconFlow API max retries exceeded")


def _embed_hf(texts: List[str], retries: int = 3) -> List[List[float]]:
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code == 429 and attempt < retries - 1:
                ...
                time.sleep(wait)
                continue
            raise RuntimeError(f"HuggingFace API error {e.code}: {detail}") from e
        except Exception as e:
            if attempt < retries - 1:
                print(f"  重试 ({attempt + 1}/{retries}): {type(e).__name__}", flush=True)
                time.sleep(5)
                continue
            raise
...
def embed_batched(texts: List[str], batch_size: int = 8, desc: str = "embedding") -> List[List[float]]:
    all_vecs = []
    total = len(texts)
    for i in range(0, total, batch_size):
        batch = texts[i:i + batch_size]
        msg = f"  {desc}: {i + 1}-{min(i + batch_size, total)}/{total}"
        print(msg, flush=True)
        try:
            vecs = sf_embed(batch)
            all_vecs.extend(vecs)
        except Exception as e:
            print(f"  ERROR at batch {i}: {type(e).__name__}: {e}", flush=True)
            raise
...
def get_doc_embeddings(docs: List[Doc], force: bool = False) -> List[List[float]]:
    ...
    if cache_path.exists() and not force:
        ...
        with cache_path.open("r", encoding="utf-8") as f:
            return json.load(f)
    print("计算文档向量 (bge-m3 via HF/SiliconFlow API)...")
    texts = [doc.text[:200] for doc in docs]
    vecs = embed_batched(texts, batch_size=8, desc="doc")
    with cache_path.open("w", encoding="utf-8") as f:
        json.dump(vecs, f)
    print(f"文档向量已缓存到 {cache_path}")
    return vecs


def embed_query(query: str) -> List[float]:
    return sf_embed([query])[0]
...
def main():
    top_k = 5
    print("=" * 70)
    print("  字幕检索对比实验：BM25 vs bge-m3 向量检索")
    print("=" * 70)

    print("\n[1/5] 加载数据...")
    docs = load_documents(limit=500)
    print(f"  文档数: {len(docs)} (采样前500条)")
    kg = load_kg()
    print(f"  图谱词条: {len(kg)}")
