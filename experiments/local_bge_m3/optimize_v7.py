...
class BM25:
    ...
    def score_doc(self, query, doc_idx, terms=None, boost_terms=None, boost_weight=0.5):
        q_toks = tokenize(query, list(self.extra) + list(terms or []))
        if not q_toks:
            return 0.0
        qc = Counter(q_toks)
...
def main():
    ...
    cand = 80
    print("=" * 70, flush=True)
    print("  优化实验 V7: Vec优先回退 + 置信度门控 + 双路KG增强", flush=True)
    print("=" * 70, flush=True)
    ...
    def wsf_then_vec_fallback(q, kw, kg_):
        wsf_result = wsf_fusion(
            vector_search(embed_query(q), vecs, top_k=cand),
            bm25.search(q, top_k=cand),
            w_a=0.5, w_b=0.5, top_n=top_k)

        bm25_top = bm25.search(q, top_k=10)
        bm25_top_score = bm25_top[0][1] if bm25_top else 0

        vec_top = vector_search(embed_query(q), vecs, top_k=10)
        vec_top_score = vec_top[0][1] if vec_top else 0

        if bm25_top_score < 30.0 and vec_top_score > 0.45:
            return vec_top[:top_k]

        return wsf_result

    experiments.append(("wsf_vec_fallback", wsf_then_vec_fallback))

    def wsf_vec_fallback_v2(q, kw, kg_):
        wsf_result = wsf_fusion(
            vector_search(embed_query(q), vecs, top_k=cand),
            bm25.search(q, top_k=cand),
            w_a=0.5, w_b=0.5, top_n=top_k)
        ...
        vec_exp_top = vector_search(embed_query(eq), vecs, top_k=10)
        ...
        vec_exp_conf = vec_exp_top[0][1] if vec_exp_top else 0

        best_vec = vec_top if vec_conf >= vec_exp_conf else vec_exp_top
        best_vec_conf = max(vec_conf, vec_exp_conf)

        if bm25_conf < 0.4 and best_vec_conf > 0.45:
            return best_vec[:top_k]

        return wsf_result

    experiments.append(("wsf_vec_fallback_v2", wsf_vec_fallback_v2))

    def smart_merge(q, kw, kg_):
        bm25_top = bm25.search(q, top_k=cand)
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
        eq = kg_expanded_queries.get(q, q)
        vec_exp_top = vector_search(embed_query(eq), vecs, top_k=cand)
        ...
        if bm25_conf < 0.4 and best_vec_conf > 0.4:
            return best_vec[:top_k]

        if best_vec_conf < 0.35 and bm25_conf > 0.6:
            return bm25_top[:top_k]

        return wsf_fusion(best_vec, bm25_top, w_a=0.5, w_b=0.5, top_n=top_k)

    experiments.append(("smart_merge", smart_merge))

    def smart_merge_kg(q, kw, kg_):
        _, bt, _ = match_kg(q, kg_)
        bm25_top = bm25.search(q, top_k=cand, boost_terms=bt, boost_weight=0.3)
        eq = kg_expanded_queries.get(q, q)
        vec_exp_top = vector_search(embed_query(eq), vecs, top_k=cand)
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
    ...
    experiments.append(("smart_merge_kg", smart_merge_kg))

    def smart_merge_relaxed(q, kw, kg_):
        bm25_top = bm25.search(q, top_k=cand)
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
        eq = kg_expanded_queries.get(q, q)
        vec_exp_top = vector_search(embed_query(eq), vecs, top_k=cand)
        ...
        if bm25_conf < 0.5 and best_vec_conf > 0.35:
            return best_vec[:top_k]

        if best_vec_conf < 0.4 and bm25_conf > 0.5:
            return bm25_top[:top_k]

        return wsf_fusion(best_vec, bm25_top, w_a=0.5, w_b=0.5, top_n=top_k)

    experiments.append(("smart_merge_relaxed", smart_merge_relaxed))

    def multi_vec_smart(q, kw, kg_):
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
        eq = kg_expanded_queries.get(q, q)
        vec_exp_top = vector_search(embed_query(eq), vecs, top_k=cand)
        bm25_top = bm25.search(q, top_k=cand)
        ...
        vec_set = {idx for idx, _ in vec_top[:20]}
        vec_exp_set = {idx for idx, _ in vec_exp_top[:20]}

        max_v = max((s for _, s in vec_top), default=1.0) or 1.0
        max_ve = max((s for _, s in vec_exp_top), default=1.0) or 1.0

        combined = {}
        for idx, s in vec_top:
            combined[idx] = combined.get(idx, 0) + 0.5 * (s / max_v)
        for idx, s in vec_exp_top:
            combined[idx] = combined.get(idx, 0) + 0.5 * (s / max_ve)

        for idx in vec_set & vec_exp_set:
            combined[idx] = combined.get(idx, 0) + 0.2

        if bm25_conf < 0.5:
            merged = sorted(combined.items(), key=lambda x: x[1], reverse=True)
            return merged[:top_k]

        bm25_norm = {idx: s / max_bm25 for idx, s in bm25_top}
        for idx in combined:
            combined[idx] = combined[idx] * 0.6 + bm25_norm.get(idx, 0) * 0.4

        merged = sorted(combined.items(), key=lambda x: x[1], reverse=True)
        return merged[:top_k]

    experiments.append(("multi_vec_smart", multi_vec_smart))

    def cascade_smart(q, kw, kg_):
        bm25_top = bm25.search(q, top_k=cand)
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
        eq = kg_expanded_queries.get(q, q)
        vec_exp_top = vector_search(embed_query(eq), vecs, top_k=cand)
        ...
        if bm25_conf < 0.4 and max(vec_conf, vec_exp_conf) > 0.4:
            return best_vec[:top_k]

        if max(vec_conf, vec_exp_conf) < 0.35 and bm25_conf > 0.6:
            return bm25_top[:top_k]

        wsf = wsf_fusion(best_vec, bm25_top, w_a=0.5, w_b=0.5, top_n=top_k)
        rrf = rrf_fusion(best_vec, bm25_top, k=60, w_a=1.0, w_b=2.0, top_n=top_k)

        wsf_ids = {idx for idx, _ in wsf[:3]}
        rrf_ids = {idx for idx, _ in rrf[:3]}

        if len(wsf_ids & rrf_ids) >= 2:
            return wsf
        else:
            seen = {}
            rank = 0
            for i in range(max(len(wsf), len(rrf))):
                if i < len(wsf) and wsf[i][0] not in seen:
                    seen[wsf[i][0]] = rank
                    rank += 1
                if i < len(rrf) and rrf[i][0] not in seen:
                    seen[rrf[i][0]] = rank
                    rank += 1
                if len(seen) >= top_k:
                    break
    ...
    experiments.append(("cascade_smart", cascade_smart))

    for k_val in [30, 60]:
        for wa in [1.0, 1.5, 2.0]:
            for wb in [1.0, 1.5, 2.0]:
                if wa == wb == 1.0:
                    continue
                def make_rrf(k_, wa_, wb_):
                    def fn(q, kw, kg_):
                        return rrf_fusion(
                            vector_search(embed_query(q), vecs, top_k=cand),
                            bm25.search(q, top_k=cand),
                            k=k_, w_a=wa_, w_b=wb_, top_n=top_k)
                    return fn
                experiments.append((f"RRF(k{k_val},v{wa},b{wb})", make_rrf(k_val, wa, wb)))
    ...
    print_table(headers, rows, title="优化实验 V7 结果")

    print(f"\n对比基线: BM25=0.769  BM25+KG=0.867  V5/V6最优=0.900", flush=True)
    ...
    out_path = Path(__file__).resolve().parent / "optimize_v7_results.json"
