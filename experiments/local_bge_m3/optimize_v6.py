...
def main():
    ...
    cand = 80
    print("=" * 70, flush=True)
    print("  优化实验 V6: 证据投票 + Vec优先回退 + 深度融合", flush=True)
    print("=" * 70, flush=True)
    ...
    def evidence_vote_kg(q, kw, kg_):
        ...
        for idx in all_ids:
            b_score = bm25_norm.get(idx, 0)
            v_score = vec_norm.get(idx, 0)
            vote_bonus = 0.3 if (idx in bm25_set and idx in vec_set) else 0.0
            combined[idx] = 0.5 * b_score + 0.5 * v_score + vote_bonus

        merged = sorted(combined.items(), key=lambda x: x[1], reverse=True)
        return merged[:top_k]

    experiments.append(("evidence_vote_kg", evidence_vote_kg))

    def vec_dominant(q, kw, kg_):
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
        bm25_top = bm25.search(q, top_k=cand)
        ...
        bm25_confidence = (bm25_top[0][1] / max_bm25) if bm25_top else 0
        vec_confidence = (vec_top[0][1] / max_vec) if vec_top else 0

        if vec_confidence > 0.8 and bm25_confidence < 0.5:
            return vec_top[:top_k]

        return wsf_fusion(vec_top, bm25_top, w_a=0.6, w_b=0.4, top_n=top_k)

    experiments.append(("vec_dominant", vec_dominant))

    def vec_dominant_v2(q, kw, kg_):
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
        bm25_top = bm25.search(q, top_k=cand)
        eq = kg_expanded_queries.get(q, q)
        ...
        max_bm25 = max((s for _, s in bm25_top), default=1.0) or 1.0
        max_vec = max((s for _, s in vec_top), default=1.0) or 1.0
        max_vec_exp = max((s for _, s in vec_exp_top), default=1.0) or 1.0

        bm25_conf = (bm25_top[0][1] / max_bm25) if bm25_top else 0
        vec_conf = (vec_top[0][1] / max_vec) if vec_top else 0
        vec_exp_conf = (vec_exp_top[0][1] / max_vec_exp) if vec_exp_top else 0

        if vec_exp_conf > vec_conf:
            vec_primary = vec_exp_top
            vec_conf = vec_exp_conf
        else:
            vec_primary = vec_top

        if vec_conf > 0.8 and bm25_conf < 0.5:
            return vec_primary[:top_k]

        if bm25_conf > 0.8 and vec_conf < 0.4:
            return bm25_top[:top_k]

        return wsf_fusion(vec_primary, bm25_top, w_a=0.6, w_b=0.4, top_n=top_k)

    experiments.append(("vec_dominant_v2", vec_dominant_v2))

    def rerank_cascade(q, kw, kg_):
        bm25_top = bm25.search(q, top_k=cand)
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)

        candidate_ids = set()
        for idx, _ in vec_top[:30]:
            candidate_ids.add(idx)
        for idx, _ in bm25_top[:30]:
            candidate_ids.add(idx)

        max_bm25 = max((s for _, s in bm25_top), default=1.0) or 1.0
        max_vec = max((s for _, s in vec_top), default=1.0) or 1.0

        bm25_map = {idx: s / max_bm25 for idx, s in bm25_top}
        vec_map = {idx: s / max_vec for idx, s in vec_top}

        rescored = []
        for idx in candidate_ids:
            b = bm25_map.get(idx, 0)
            v = vec_map.get(idx, 0)
            combined = 0.4 * b + 0.6 * v
            if idx in bm25_map and idx in vec_map:
                combined += 0.2
    ...
    experiments.append(("rerank_cascade", rerank_cascade))

    def rerank_cascade_v2(q, kw, kg_):
        bm25_top = bm25.search(q, top_k=cand)
        vec_top = vector_search(embed_query(q), vecs, top_k=cand)
        eq = kg_expanded_queries.get(q, q)
        vec_exp_top = vector_search(embed_query(eq), vecs, top_k=cand)
        ...
        bm25_map = {idx: s / max_bm25 for idx, s in bm25_top}
        vec_map = {idx: s / max_vec for idx, s in vec_top}
        vec_exp_map = {idx: s / max_vec_exp for idx, s in vec_exp_top}

        rescored = []
        for idx in candidate_ids:
            b = bm25_map.get(idx, 0)
            v = vec_map.get(idx, 0)
            ve = vec_exp_map.get(idx, 0)
            best_vec = max(v, ve)
            combined = 0.4 * b + 0.6 * best_vec
            if idx in bm25_map and (idx in vec_map or idx in vec_exp_map):
                combined += 0.15
            rescored.append((idx, combined))
    ...
    experiments.append(("rerank_cascade_v2", rerank_cascade_v2))

    def rrf_large_cand(q, kw, kg_):
        return rrf_fusion(
            vector_search(embed_query(q), vecs, top_k=cand),
            bm25.search(q, top_k=cand),
            k=60, w_a=1.0, w_b=2.0, top_n=top_k)

    experiments.append(("RRF(k60,b2,v1,c80)", rrf_large_cand))
        ...
        for wa in [1.0, 1.5]:
            for wb in [1.5, 2.0]:
                def make_rrf(k_, wa_, wb_):
                    ...
                    return fn
                experiments.append((f"RRF(k{k_val},b{wb},v{wa},c{cand})", make_rrf(k_val, wa, wb)))

    def wsf_kg_expand_boost(q, kw, kg_):
        eq = kg_expanded_queries.get(q, q)
        _, bt, _ = match_kg(q, kg_)
        vec_top = vector_search(embed_query(eq), vecs, top_k=cand)
        bm25_top = bm25.search(q, top_k=cand, boost_terms=bt, boost_weight=0.3)
    ...
    experiments.append(("rerank_kg_expand_boost", wsf_kg_expand_boost))
    ...
    print_table(headers, rows, title="优化实验 V6 结果")
    ...
    out_path = Path(__file__).resolve().parent / "optimize_v6_results.json"
